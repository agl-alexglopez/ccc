...
static void build_graph(struct graph *const graph) {
  ...
  for (int vertex = 0, vertex_title = start_vertex_title;
    ...
    *grid_at_mut(graph, rand_point) =
        vertex_bit | path_bit | ((Cell)vertex_title << vertex_cell_title_shift);
...
static bool connect_random_edge(struct graph *const graph,
                                struct vertex *const src_vertex) {
...
static bool find_grid_vertex_bfs(struct graph *const graph,
  ...
  while (!q_empty(&bfs)) {
    ...
    for (size_t i = 0; i < dirs_size; ++i) {
      ...
      if (is_dst(next_cell, dst->name) ||
          (!is_path(next_cell) &&
           !set_contains(&parent_map, &push.elem, cmp_parent_cells, NULL))) {
...
static struct point random_vertex_placement(const struct graph *const graph) {
...
static inline bool is_dst(const Cell c, const char dst) {
  return is_vertex(c) && get_cell_vertex_title(c) == dst;
}

/*========================    Graph Solving    ==============================*/

static void find_shortest_paths(struct graph *const graph) {
  char *lineptr = NULL;
  for (;;) {
    clear_paint(graph);
...
static bool dijkstra_shortest_path(struct graph *const graph,
                                   const struct path_request pr) {
  ...
  if (success) {
    ...
    struct prev_vertex *prev =
        set_entry(set_find(&prev_map, &key.elem, cmp_set_prev_vertices, NULL),
...
static void paint_edge(struct graph *const g, const struct vertex *const src,
...
static inline bool is_valid_vertex_pos(const struct graph *graph,
                                       struct point p) {
...
static int vertex_degree(const struct vertex *const v) {
  int n = 0;
...
static inline Cell *grid_at_mut(const struct graph *const graph,
                                struct point p) {
  return &graph->grid[p.r * graph->cols + p.c];
}

static inline Cell grid_at(const struct graph *const graph, struct point p) {
  return graph->grid[p.r * graph->cols + p.c];
}

static inline uint16_t sort_vertices(char a, char b) {
  return a < b ? a << 8 | b : b << 8 | a;
}

static char get_cell_vertex_title(const Cell cell) {
  return (char)((cell & vertex_title_mask) >> vertex_cell_title_shift);
}

static bool has_edge_with(const struct vertex *const v, char vertex) {
...
static bool add_edge(struct vertex *const v, const struct edge *const e) {
...
static inline bool is_vertex(Cell c) { return (c & vertex_bit) != 0; }

static bool is_path(Cell c) { return (c & path_bit) != 0; }

static void clear_and_flush_graph(const struct graph *const g) {
  clear_screen();
...
static void clear_paint(struct graph *const graph) {
...
static void print_cell(const Cell cell) {

  if (cell & vertex_bit) {
    printf("\033[38;5;14m%c\033[0m", (char)(cell >> vertex_cell_title_shift));
...
static bool is_edge_vertex(const Cell square, Cell edge_id) {
...
static bool is_valid_edge_cell(const Cell square, const Cell edge_id) {
  return ((square & vertex_bit) && is_edge_vertex(square, edge_id)) ||
         ((square & path_bit) && (square & edge_id_mask) == edge_id);
}

static void build_path_cell(struct graph *g, struct point p,
                            const Cell edge_id) {
  Cell path = path_bit;
  if (p.r - 1 >= 0 &&
      is_valid_edge_cell(grid_at(g, (struct point){.r = p.r - 1, .c = p.c}),
                         edge_id)) {
    path |= north_path;
  ...
  if (p.c - 1 >= 0 &&
      is_valid_edge_cell(grid_at(g, (struct point){.r = p.r, .c = p.c - 1}),
                         edge_id)) {
    path |= west_path;
...
static bool insert_prev_vertex(struct set *s, struct prev_vertex pv) {
...
static bool insert_parent_cell(struct set *s, struct parent_cell pc) {
  struct parent_cell *const pc_heap = valid_malloc(sizeof(struct parent_cell));
  *pc_heap = pc;
  return set_insert(s, &pc_heap->elem, cmp_parent_cells, NULL);
}

static threeway_cmp cmp_vertices(const struct set_elem *const a,
                                 const struct set_elem *const b, void *aux) {
  (void)aux;
...
static threeway_cmp cmp_parent_cells(const struct set_elem *x,
                                     const struct set_elem *y, void *aux) {
  (void)aux;
...
static threeway_cmp cmp_pq_dist_points(const struct pq_elem *const x,
                                       const struct pq_elem *const y,
                                       void *aux) {
  (void)aux;
...
static threeway_cmp cmp_set_prev_vertices(const struct set_elem *const x,
                                          const struct set_elem *const y,
                                          void *aux) {
  (void)aux;
...
static void pq_update_dist(struct pq_elem *e, void *aux) {
  pq_entry(e, struct dist_point, pq_elem)->dist = *((int *)aux);
}

static void print_vertex(const struct set_elem *const x) {
...
static void set_vertex_destructor(struct set_elem *const e) {
  free(set_entry(e, struct vertex, elem));
}

static void set_pq_prev_vertex_dist_point_destructor(struct set_elem *const e) {
  struct prev_vertex *pv = set_entry(e, struct prev_vertex, elem);
  free(pq_entry(pv->pq_elem, struct dist_point, pq_elem));
  free(pv);
}

static void set_parent_point_destructor(struct set_elem *const e) {
  free(set_entry(e, struct parent_cell, elem));
}

/*===========================    Misc    ====================================*/

static struct path_request parse_path_request(struct graph *const g,
                                              str_view r) {
  ...
  for (const char *c = sv_begin(r); c != sv_end(r); c = sv_next(c)) {
    if (*c >= start_vertex_title && *c <= end_title) {
      ...
      struct vertex *v =
          set_entry(set_find(&g->adjacency_list, &key.elem, cmp_vertices, NULL),
                    struct vertex, elem);
...
static struct int_conversion parse_digits(str_view arg) {
...
static void *valid_malloc(size_t n) {
  void *mem = malloc(n);
  if (!mem) {
