...
still valid on 32-bit but probably very slow. */
enum
{
#if defined(__SSE2__)
...
should be fine as they are likely to only declare one or two */
#define ccc_impl_shm_declare_fixed_map(fixed_map_type_name, key_val_type_name, \
                                       capacity)                               \
    struct fixed_map_type_name                                                 \
    {                                                                          \
        enum ccc_shm_meta                                                      \
