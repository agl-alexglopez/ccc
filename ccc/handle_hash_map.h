...
[[nodiscard]] ptrdiff_t ccc_hhm_size(ccc_handle_hash_map const *h);
...
[[nodiscard]] ptrdiff_t ccc_hhm_next_prime(ptrdiff_t n);

/** @brief Return the full capacity of the backing storage.
@param [in] h the hash table.
@return the capacity or -1 if h is NULL. */
[[nodiscard]] ptrdiff_t ccc_hhm_capacity(ccc_handle_hash_map const *h);
