...
An adaptive map offers storage and retrieval elements sorted on the user
specified key. Because the data structure is self-optimizing it is not suitable
map in a realtime environment where strict runtime bounds are needed. Also,
searching the map is not a const thread-safe operation as indicated by the
function signatures. The map is optimized upon every new search attempting to
adapt to the usage pattern. In many cases the self-optimizing structure of the
map can be beneficial when considering non-uniform access patterns. In the best
case, repeated searches of the same value yield an O(1) access and many other
frequently searched values will remain close to the root
...
@param [in] map_node_field the name of the intrusive map elem field.
...
#define CCC_adaptive_map_initialize(map_name, struct_name, map_node_field,     \
                                    ...
                                    context)                                   \
    CCC_private_adaptive_map_initialize(map_name, struct_name, map_node_field, \
                                        key_node_field, key_order, allocate,   \
                                        context)
...
CCC_adaptive_map_remove(CCC_Adaptive_map *map,
                        CCC_Adaptive_map_node *out_handle);
...
*/
[[nodiscard]] CCC_Adaptive_map_entry *
CCC_adaptive_map_and_modify(CCC_Adaptive_map_entry *e, CCC_Type_modifier *fn);
...
[[nodiscard]] CCC_Adaptive_map_entry *
CCC_adaptive_map_and_modify_context(CCC_Adaptive_map_entry *e,
...
```
#define ADAPTIVE_MAP_USING_NAMESPACE_CCC
map_entry *e = adaptive_map_and_modify_w(entry_r(&map, &k), word, T->cnt++;);
word *w = adaptive_map_or_insert_w(adaptive_map_and_modify_w(entry_r(&map, &k),
...
[[nodiscard]] void *CCC_adaptive_map_or_insert(CCC_Adaptive_map_entry const *e,
...
[[nodiscard]] void *
CCC_adaptive_map_insert_entry(CCC_Adaptive_map_entry const *e,
                              CCC_Adaptive_map_node *elem);
...
[[nodiscard]] void *CCC_adaptive_map_unwrap(CCC_Adaptive_map_entry const *e);
...
CCC_adaptive_map_occupied(CCC_Adaptive_map_entry const *e);
...
NULL. */
[[nodiscard]] CCC_Tribool
CCC_adaptive_map_insert_error(CCC_Adaptive_map_entry const *e);
...
CCC_adaptive_map_entry_status(CCC_Adaptive_map_entry const *e);
...
CCC_adaptive_map_next(CCC_Adaptive_map const *map,
                      CCC_Adaptive_map_node const *iter_handle);
