                      ...
                      ccc_homap_elem *const elem)
{
    ...
    if (h->impl_.handle_.stats_ == CCC_ENTRY_OCCUPIED)
    {
        *elem = *at(h->impl_.hom_, h->impl_.handle_.i_);
        ...
        if (e_base != ret)
        {
            memcpy(ret, e_base, ccc_buf_elem_size(&h->impl_.hom_->buf_).count);
        }
                    ...
                    ccc_homap_elem *const out_handle)
{
    ...
    if (found)
    {
        ...
        *out_handle = *at(hom, hom->root_);
        ...
        swap(tmp, user_struct, ret, ccc_buf_elem_size(&hom->buf_).count);
                         ...
                         ccc_homap_elem *const key_val_handle)
{
    ...
    if (found)
    {
        *key_val_handle = *at(hom, found);
        ...
        if (e_base != f_base)
        {
            memcpy(f_base, e_base, ccc_buf_elem_size(&hom->buf_).count);
        }
        return (ccc_handle){{.i_ = found, .stats_ = CCC_ENTRY_OCCUPIED}};
    }
    size_t const inserted = maybe_alloc_insert(hom, key_val_handle);
...
ccc_hom_size(ccc_handle_ordered_map const *const hom)
{
    ...
    ccc_ucount sz = ccc_buf_size(&hom->buf_);
...
insert(struct ccc_homap_ *const t, size_t const n)
{
    ...
    init_node(node);
    if (ccc_buf_size(&t->buf_).count == empty_tree)
    {
        t->root_ = n;
        return;
    }
...
{
    return ccc_buf_i(&t->buf_, struct_base(t, elem)).count;
}
...
is_free_list_valid(struct ccc_homap_ const *const t)
{
    ...
    for (size_t cur = t->free_list_;
         cur && list_check < ccc_buf_capacity(&t->buf_).count;
         cur = at(t, cur)->next_free_, ++list_check)
    {}
    return (list_check + ccc_buf_size(&t->buf_).count
...
validate(struct ccc_homap_ const *const hom)
{
    ...
    if (size && size != ccc_buf_size(&hom->buf_).count - 1)
    {
        return CCC_FALSE;
    }
