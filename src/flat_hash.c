...
static void erase(struct ccc_impl_fhash *, void *);
...
static bool valid_distance_from_home(struct ccc_impl_fhash const *,
                                     void const *slot);
                 ...
                 ccc_key_cmp_fn *const eq_fn, void *const aux)
{
    ...
    h->eq_fn = eq_fn;
    h->aux = aux;
    memset(ccc_buf_begin(&h->buf), EMPTY,
...
{
    if (e.entry.status == CCC_ENT_OCCUPIED)
    {
...
ccc_fh_insert(ccc_fhash *h, void *const key, ccc_fhash_elem *const out_handle)
{
    ...
    if (ent.entry.status & CCC_ENT_OCCUPIED)
    {
...
ccc_fh_or_insert(ccc_fhash_entry e, ccc_fhash_elem *const elem)
{
    ...
    if (e.impl.entry.status & ~CCC_ENT_VACANT)
    {
        return NULL;
    }
    void *user_struct = struct_base(e.impl.h, &elem->impl);
...
{
    return e.impl.entry.status & CCC_ENT_OCCUPIED;
}

inline bool
ccc_fh_insert_error(ccc_fhash_entry e)
{
    return e.impl.entry.status & CCC_ENT_INS_ERROR;
}
...
entry(struct ccc_impl_fhash *const h, void const *key, uint64_t const hash)
{
    ...
    {
        upcoming_insertion_error = CCC_ENT_INS_ERROR;
    }
                 ...
                 uint64_t const hash)
{
    ...
    for (size_t dist = 0; dist < cap; ++dist, cur_i = (cur_i + 1) % cap)
    {
        ...
        if (e->hash == EMPTY)
        {
            return (ccc_entry){.entry = slot, .status = CCC_ENT_VACANT};
        }
        ...
        {
            return (ccc_entry){.entry = slot, .status = CCC_ENT_OCCUPIED};
        }
    }
    return (ccc_entry){.entry = NULL, .status = CCC_ENT_VACANT | CCC_ENT_NULL};
}
...
static void
erase(struct ccc_impl_fhash *const h, void *const e)
{
    size_t const cap = ccc_buf_capacity(&h->buf);
    size_t const elem_sz = ccc_buf_elem_size(&h->buf);
...
bool
ccc_fh_validate(ccc_fhash const *const h)
{
    for (void const *i = ccc_buf_begin(&h->impl.buf);
         i != ccc_buf_capacity_end(&h->impl.buf);
         i = ccc_buf_next(&h->impl.buf, i))
...
static bool
valid_distance_from_home(struct ccc_impl_fhash const *h, void const *slot)
{
    size_t const cap = ccc_buf_capacity(&h->buf);
