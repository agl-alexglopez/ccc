...
static void *push_range(struct ccc_fdeq_ *fq, char const *pos, size_t n,
                        char const *elems);
static void *alloc_front(struct ccc_fdeq_ *fq);
static void *alloc_back(struct ccc_fdeq_ *fq);
                   ...
                   void const *const elem)
{
    ...
    void *const slot = alloc_back(fq);
    if (slot)
    {
        (void)memcpy(slot, elem, buf_elem_size(&fq->buf_));
    }
    return slot;
}
                    ...
                    void const *const elem)
{
    ...
    void *const slot = alloc_front(fq);
    if (slot)
    {
        (void)memcpy(slot, elem, buf_elem_size(&fq->buf_));
    }
...
void *
ccc_fdeq_insert_range(ccc_flat_double_ended_queue *const fq, void *pos,
...
{
    if (buf_is_empty(&fq->buf_))
    {
        return CCC_INPUT_ERR;
    }
    fq->front_ = increment(fq, fq->front_);
    return buf_size_minus(&fq->buf_, 1);
}
...
ccc_fdeq_front(ccc_flat_double_ended_queue const *const fq)
{
    ...
    return buf_at(&fq->buf_, fq->front_);
}
...
ccc_fdeq_at(ccc_flat_double_ended_queue const *const fq, size_t const i)
{
    ...
    return buf_at(&fq->buf_, (fq->front_ + i) % buf_capacity(&fq->buf_));
}

void *
ccc_fdeq_begin(ccc_flat_double_ended_queue const *const fq)
{
...
void *
ccc_fdeq_rbegin(ccc_flat_double_ended_queue const *const fq)
{
    if (!buf_size(&fq->buf_))
    {
        return NULL;
...
void *
ccc_fdeq_next(ccc_flat_double_ended_queue const *const fq, void const *iter_ptr)
{
    size_t const next_i = increment(fq, index_of(fq, iter_ptr));
    if (next_i == fq->front_
        || distance(fq, next_i, fq->front_) >= buf_size(&fq->buf_))
    {
        return NULL;
    }
    return buf_at(&fq->buf_, next_i);
}

void *
ccc_fdeq_rnext(ccc_flat_double_ended_queue const *const fq,
               void const *iter_ptr)
{
    ...
    if (next_i == rbegin
        || rdistance(fq, next_i, rbegin) >= buf_size(&fq->buf_))
    {
        return NULL;
    }
    return buf_at(&fq->buf_, next_i);
...
ccc_fdeq_validate(ccc_flat_double_ended_queue const *const fq)
{
    ...
    if (buf_i(&fq->buf_, iter) != (ptrdiff_t)fq->front_)
    {
        return false;
    }
    size_t size = 0;
    ...
    if (buf_i(&fq->buf_, iter) != (ptrdiff_t)last_elem_index(fq))
    {
        return false;
    }
               ...
               ccc_destructor_fn *destructor)
{
    ...
    {
        fq->front_ = 0;
        return buf_size_set(&fq->buf_, 0);
    }
    size_t const back = back_free_slot(fq);
    for (size_t i = fq->front_; i != back; i = increment(fq, i))
    {
        destructor((ccc_user_type_mut){.user_type = buf_at(&fq->buf_, i),
                                       .aux = fq->aux_});
    }
    return CCC_OK;
}
                        ...
                        ccc_destructor_fn *destructor)
{
    ...
    if (!destructor)
    {
        ...
        return buf_alloc(&fq->buf_, 0, fq->buf_.alloc_);
    }
    size_t const back = back_free_slot(fq);
    for (size_t i = fq->front_; i != back; i = increment(fq, i))
    {
...
void *
ccc_impl_fdeq_alloc_back(struct ccc_fdeq_ *const fq)
{
    return alloc_back(fq);
}

void *
ccc_impl_fdeq_alloc_front(struct ccc_fdeq_ *const fq)
{
    return alloc_front(fq);
}

/*======================     Static Helpers    ==============================*/

static inline void *
alloc_front(struct ccc_fdeq_ *const fq)
{
    void *new_slot = NULL;
    ...
    fq->front_ = front_free_slot(fq->front_, buf_capacity(&fq->buf_));
    new_slot = buf_at(&fq->buf_, fq->front_);
    if (!full)
    {
        buf_size_plus(&fq->buf_, 1);
    }
    return new_slot;
}

static inline void *
alloc_back(struct ccc_fdeq_ *const fq)
{
    void *new_slot = NULL;
    bool const full = maybe_resize(fq, 0) != CCC_OK;
    /* Should have been able to resize. Bad error. */
    ...
    new_slot = buf_at(&fq->buf_, back_free_slot(fq));
...
{
    size_t const elem_sz = buf_elem_size(&fq->buf_);
    bool const full = maybe_resize(fq, n) != CCC_OK;
    size_t const cap = buf_capacity(&fq->buf_);
    if (fq->buf_.alloc_ && full)
    {
        return CCC_MEM_ERR;
    }
    ...
    if (n >= cap)
    {
        elems += ((n - cap) * elem_sz);
        fq->front_ = 0;
        (void)memcpy(buf_at(&fq->buf_, 0), elems, elem_sz * cap);
        (void)buf_size_set(&fq->buf_, cap);
        return CCC_OK;
    }
    size_t const new_size = buf_size(&fq->buf_) + n;
    ...
    (void)memcpy(buf_at(&fq->buf_, back_slot), elems, chunk * elem_sz);
    if (remainder)
    {
        (void)memcpy(buf_at(&fq->buf_, remainder_back_slot), second_chunk,
                     remainder * elem_sz);
    }
    if (new_size > cap)
    {
    ...
    (void)buf_size_set(&fq->buf_, MIN(cap, new_size));
    return CCC_OK;
}
...
push_front_range(struct ccc_fdeq_ *const fq, size_t const n, char const *elems)
{
    ...
    (void)memcpy(buf_at(&fq->buf_, i), first_chunk, chunk * elem_sz);
    if (remainder)
    {
        (void)memcpy(buf_at(&fq->buf_, cap - remainder), elems,
                     remainder * elem_sz);
    }
    (void)buf_size_set(&fq->buf_, MIN(cap, buf_size(&fq->buf_) + n));
           ...
           char const *elems)
{
    ...
    (void)buf_size_set(&fq->buf_, MIN(cap, new_size));
    return buf_at(&fq->buf_, pos_i);
}

static inline ccc_result
maybe_resize(struct ccc_fdeq_ *const q, size_t const additional_elems_to_add)
{
    if (buf_size(&q->buf_) + additional_elems_to_add < buf_capacity(&q->buf_))
    {
        return CCC_OK;
    }
    if (!q->buf_.alloc_)
    ...
    size_t const elem_sz = buf_elem_size(&q->buf_);
    size_t const new_cap
        = buf_capacity(&q->buf_)
              ? ((buf_capacity(&q->buf_) + additional_elems_to_add) * 2)
              : start_capacity;
    void *new_mem = q->buf_.alloc_(NULL, buf_elem_size(&q->buf_) * new_cap);
    if (!new_mem)
    {
        return CCC_MEM_ERR;
    }
    size_t const first_chunk
        = MIN(buf_size(&q->buf_), buf_capacity(&q->buf_) - q->front_);
    (void)memcpy(new_mem, buf_at(&q->buf_, q->front_), elem_sz * first_chunk);
    if (first_chunk < buf_size(&q->buf_))
    {
        (void)memcpy((char *)new_mem + (elem_sz * first_chunk),
                     buf_begin(&q->buf_),
                     elem_sz * (buf_size(&q->buf_) - first_chunk));
    }
    (void)buf_alloc(&q->buf_, 0, q->buf_.alloc_);
    q->buf_.mem_ = new_mem;
    q->front_ = 0;
         ...
         size_t const origin)
{
    return iter > origin ? iter - origin
                         : (buf_capacity(&fq->buf_) - origin) + iter;
}
...
rdistance(struct ccc_fdeq_ const *const fq, size_t const iter,
          size_t const origin)
{
    return iter > origin ? (buf_capacity(&fq->buf_) - iter) + origin
                         : origin - iter;
}
...
{
    assert(pos >= buf_begin(&fq->buf_));
    assert(pos < buf_capacity_end(&fq->buf_));
    return ((char *)pos - (char *)buf_begin(&fq->buf_))
           / buf_elem_size(&fq->buf_);
}

static void *
at(struct ccc_fdeq_ const *const fq, size_t const i)
{
    if (i >= buf_capacity(&fq->buf_))
    {
        return NULL;
    }
    return buf_at(&fq->buf_, (fq->front_ + i) % buf_capacity(&fq->buf_));
...
{
    return i == (buf_capacity(&fq->buf_) - 1) ? 0 : ++i;
}

size_t
decrement(struct ccc_fdeq_ const *const fq, size_t i)
{
    return i ? --i : buf_capacity(&fq->buf_) - 1;
}

static inline size_t
back_free_slot(struct ccc_fdeq_ const *const fq)
{
    return (fq->front_ + buf_size(&fq->buf_)) % buf_capacity(&fq->buf_);
}
...
{
    return (fq->front_ + (buf_size(&fq->buf_) - 1)) % buf_capacity(&fq->buf_);
}
