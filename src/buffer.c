                ...
                ccc_alloc_fn *const fn)
{
    ...
    void *const new_mem = fn(buf->impl_.mem, new_capacity);
    if (new_capacity && !new_mem)
    {
        return CCC_MEM_ERR;
    }
    buf->impl_.mem = new_mem;
    buf->impl_.capacity = new_capacity;
    return CCC_OK;
}
...
{
    if (i >= buf->impl_.capacity)
    {
        return NULL;
    }
    return ((uint8_t *)buf->impl_.mem + (i * buf->impl_.elem_sz));
}

void *
ccc_buf_back(ccc_buffer const *buf)
{
    return ccc_buf_at(buf, buf->impl_.sz - (size_t)1);
}
...
{
    if (buf->impl_.sz != buf->impl_.capacity)
    {
        return ((uint8_t *)buf->impl_.mem
                + (buf->impl_.elem_sz * buf->impl_.sz++));
    }
    if (ccc_buf_realloc(buf, buf->impl_.capacity * 2, buf->impl_.alloc)
        == CCC_OK)
    {
...
{
    if (!buf->impl_.sz || i >= buf->impl_.sz || j >= buf->impl_.sz || j == i)
    {
        return CCC_MEM_ERR;
    }
    (void)memcpy(tmp, at(buf, i), buf->impl_.elem_sz);
    (void)memcpy(at(buf, i), at(buf, j), buf->impl_.elem_sz);
    (void)memcpy(at(buf, j), tmp, buf->impl_.elem_sz);
    return CCC_OK;
}
...
{
    if (!buf->impl_.sz || dst >= buf->impl_.sz || src >= buf->impl_.sz)
    {
        return NULL;
    }
    if (dst == src)
    ...
    return memcpy(at(buf, dst), at(buf, src), buf->impl_.elem_sz);
}
...
{
    if (!buf->impl_.sz || i >= buf->impl_.sz)
    {
        return CCC_MEM_ERR;
    }
    if (1 == buf->impl_.sz)
    ...
    (void)memcpy(at(buf, i), at(buf, i + 1),
                 buf->impl_.elem_sz * (buf->impl_.sz - (i + 1)));
    --buf->impl_.sz;
    return CCC_OK;
}
...
{
    if (!buf->impl_.capacity || !fn)
    {
        return CCC_MEM_ERR;
    }
    buf->impl_.capacity = 0;
    buf->impl_.sz = 0;
    fn(buf->impl_.mem, 0);
    return CCC_OK;
}
...
{
    return buf->impl_.capacity;
}

size_t
ccc_buf_elem_size(ccc_buffer const *buf)
{
    return buf->impl_.elem_sz;
}

bool
ccc_buf_full(ccc_buffer const *buf)
{
    return buf->impl_.sz == buf->impl_.capacity;
}
...
ccc_buf_next(ccc_buffer const *const buf, void const *const pos)
{
    ...
    return (uint8_t *)pos + buf->impl_.elem_sz;
}

void *
ccc_buf_size_end(ccc_buffer const *const buf)
{
    return (uint8_t *)buf->impl_.mem + (buf->impl_.elem_sz * buf->impl_.sz);
}
...
{
    return (uint8_t *)buf->impl_.mem
           + (buf->impl_.elem_sz * buf->impl_.capacity);
}
