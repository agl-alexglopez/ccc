...
ccc_fhm_insert_entry(ccc_fhmap_entry const *const e, ccc_fhmap_elem *const elem)
{
    ...
    insert(e->impl_.h_, user_struct, e->impl_.hash_,
           ccc_buf_i(&e->impl_.h_->buf_, e->impl_.entry_.e_).count);
    return e->impl_.entry_.e_;
}
...
ccc_fhm_and_modify(ccc_fhmap_entry *const e, ccc_update_fn *const fn)
{
    if (e && fn && e->impl_.entry_.stats_ == CCC_ENTRY_OCCUPIED
        && e->impl_.entry_.e_)
    {
        fn((ccc_user_type){e->impl_.entry_.e_, NULL});
    }
    return e;
}
...
ccc_fhm_and_modify_aux(ccc_fhmap_entry *const e, ccc_update_fn *const fn,
                       void *const aux)
{
    if (fn && e && e->impl_.entry_.stats_ == CCC_ENTRY_OCCUPIED
        && e->impl_.entry_.e_)
    {
...
ccc_fhm_swap_entry(ccc_flat_hash_map *const h, ccc_fhmap_elem *const out_handle)
{
    ...
    if (ent.entry_.stats_ & CCC_ENTRY_INSERT_ERROR || !ent.entry_.e_)
    {
        return (ccc_entry){{.stats_ = CCC_ENTRY_INSERT_ERROR}};
    }
    insert(h, user_return, ent.hash_, ccc_buf_i(&h->buf_, ent.entry_.e_).count);
    return (ccc_entry){{.stats_ = CCC_ENTRY_VACANT}};
}
                   ...
                   ccc_fhmap_elem *const key_val_handle)
{
    ...
    insert(h, user_base, ent.hash_, ccc_buf_i(&h->buf_, ent.entry_.e_).count);
...
ccc_fhm_or_insert(ccc_fhmap_entry const *const e, ccc_fhmap_elem *const elem)
{
    ...
    if (e->impl_.entry_.stats_ & CCC_ENTRY_INSERT_ERROR || !e->impl_.entry_.e_)
    {
        return NULL;
    }
    ...
    *dst = *src;
         ...
         slot = ccc_buf_next(&src->buf_, slot))
    {
        ...
        if (e->hash_ != CCC_FHM_EMPTY)
        {
            ...
            insert(dst, slot, e->hash_,
                   ccc_buf_i(&dst->buf_, new_ent.e_).count);
        }
    }
    return CCC_RESULT_OK;
}
                         ...
                         void const *const slot)
{
    ...
    for (size_t i = ccc_buf_i(&h->buf_, slot).count,
         ...
         slot = ccc_buf_next(&h->buf_, slot))
    {
        ...
        if (e->hash_ != CCC_FHM_EMPTY)
        {
            ...
            insert(&new_hash, slot, e->hash_,
                   ccc_buf_i(&new_hash.buf_, new_ent.e_).count);
        }
    }
