...
void q_init(const size_t elem_sz, struct queue *const q,
...
void q_free(struct queue *const q) {
  if (!q) {
    return;
  }
  free(q->mem);
...
void q_push(struct queue *const q, void *const elem) {
  if (!q) {
    return;
  }
  if (q->sz == q->capacity) {
...
void q_pop(struct queue *const q) {
  if (!q->sz) {
...
void *q_front(struct queue *const q) {
  if (!q->sz) {
    return NULL;
  }
...
bool q_empty(struct queue *const q) { return !q || !q->sz; }

static void q_grow(struct queue *const q) {
