...
check_static_begin(array_adaptive_map_test_remove)
{
    CCC_Array_adaptive_map array_adaptive_map
        = array_adaptive_map_initialize(&(Small_fixed_map){}, struct Val, id,
                                        id_order, NULL, NULL, SMALL_FIXED_CAP);
    int size = 30;
    CCC_Handle hndl
        = CCC_remove(&array_adaptive_map, &(struct Val){.id = -1, .val = -1});
    check(validate(&array_adaptive_map), true);
    check(occupied(&hndl), false);
    ...
    check(count(&array_adaptive_map).count, 1);
    struct Val old = {.id = -1};
    hndl = CCC_remove(&array_adaptive_map, &old);
    check(validate(&array_adaptive_map), true);
    check(occupied(&hndl), true);
    check(count(&array_adaptive_map).count, 0);
    check(old.val, -1);
    ...
    i += (size / 2);
    hndl = CCC_remove(&array_adaptive_map, &(struct Val){.id = i, .val = i});
    check(validate(&array_adaptive_map), true);
    check(occupied(&hndl), false);
...
check_static_begin(array_adaptive_map_test_array_and_modify)
{
    ...
    CCC_Array_adaptive_map_handle *hndl
        = array_wrap(&array_adaptive_map, &(int){-1});
    check(validate(&array_adaptive_map), true);
    check(occupied(hndl), false);
    check(count(&array_adaptive_map).count, 0);
    ...
    (void)array_adaptive_map_insert_or_assign_with(&array_adaptive_map, -1,
                                                   val(-1));
    check(validate(&array_adaptive_map), true);
    hndl = array_wrap(&array_adaptive_map, &(int){-1});
    check(occupied(hndl), true);
    check(count(&array_adaptive_map).count, 1);
    struct Val *v = array_adaptive_map_at(&array_adaptive_map, unwrap(hndl));
    check(v != NULL, true);
    ...
    i += (size / 2);
    hndl = array_wrap(&array_adaptive_map, &i);
    check(occupied(hndl), false);
    check(count(&array_adaptive_map).count, i + 1);
...
check_static_begin(array_adaptive_map_test_or_insert)
{
    ...
    struct Val *v = array_adaptive_map_at(
        &array_adaptive_map,
        or_insert(array_wrap(&array_adaptive_map, &(int){-1}),
                  &(struct Val){.id = -1, .val = -1}));
    check(validate(&array_adaptive_map), true);
    check(v != NULL, true);
    check(v->id, -1);
    ...
    v = array_adaptive_map_at(&array_adaptive_map,
                              or_insert(array_wrap(&array_adaptive_map, &i),
                                        &(struct Val){.id = i, .val = i}));
    check(validate(&array_adaptive_map), true);
    check(v != NULL, true);
    check(v->id, i);
...
check_static_begin(array_adaptive_map_test_or_insert_with)
{
    ...
    struct Val *v = array_adaptive_map_at(
        ...
        array_adaptive_map_or_insert_with(
            array_wrap(&array_adaptive_map, &(int){-1}), idval(-1, -1)));
    check(validate(&array_adaptive_map), true);
    check(v != NULL, true);
    check(v->id, -1);
    check(v->val, -1);
    ...
    v = array_adaptive_map_at(
        &array_adaptive_map,
        array_adaptive_map_or_insert_with(
            array_wrap(&array_adaptive_map, &(int){-1}), idval(-1, -2)));
    check(v != NULL, true);
    check(v->id, -1);
    check(v->val, -1);
    check(count(&array_adaptive_map).count, 1);
    ...
    v = array_adaptive_map_at(
        &array_adaptive_map,
        array_adaptive_map_or_insert_with(array_wrap(&array_adaptive_map, &i),
                                          idval(i, i)));
    check(validate(&array_adaptive_map), true);
    check(v != NULL, true);
    check(v->id, i);
...
check_static_begin(array_adaptive_map_test_insert_handle)
{
    ...
    struct Val *v = array_adaptive_map_at(
        &array_adaptive_map,
        insert_handle(array_wrap(&array_adaptive_map, &(int){-1}),
                      &(struct Val){.id = -1, .val = -1}));
    check(validate(&array_adaptive_map), true);
    check(v != NULL, true);
    check(v->id, -1);
...
check_static_begin(array_adaptive_map_test_insert_array_with)
{
    ...
    v = array_adaptive_map_at(
        &array_adaptive_map,
        array_adaptive_map_insert_array_with(
            array_wrap(&array_adaptive_map, &i), idval(i, i)));
    check(validate(&array_adaptive_map), true);
    check(v != NULL, true);
    check(v->id, i);
    check(v->val, i);
    ...
    v = array_adaptive_map_at(
        &array_adaptive_map,
        array_adaptive_map_insert_array_with(
            array_wrap(&array_adaptive_map, &i), idval(i, i + 1)));
    check(v != NULL, true);
    check(v->id, i);
    check(v->val, i + 1);
    check(count(&array_adaptive_map).count, i + 2);
...
check_static_begin(array_adaptive_map_test_remove_handle)
{
    ...
    check(count(&array_adaptive_map).count, 1);
    CCC_Handle *e
        = remove_array_wrap(array_wrap(&array_adaptive_map, &(int){-1}));
    check(validate(&array_adaptive_map), true);
    ...
    check(v->val, i);
    check(count(&array_adaptive_map).count, i + 1);
    e = remove_array_wrap(array_wrap(&array_adaptive_map, &i));
    check(validate(&array_adaptive_map), true);
    check(occupied(e), true);
    v = array_adaptive_map_at(&array_adaptive_map, unwrap(e));
    check(v != NULL, true);
...
main(void)
{
    return check_run(array_adaptive_map_test_insert(),
                     array_adaptive_map_test_remove(),
