...
CHECK_BEGIN_STATIC_FN(dll_test_push_three_front)
{
    doubly_linked_list dll
        = dll_initialize(dll, struct val, e, val_cmp, NULL, NULL);
    struct val v0 = {};
...
CHECK_BEGIN_STATIC_FN(dll_test_push_and_splice)
{
    ...
    CHECK(t, PASS);
    CHECK(splice(&dll, dll_elem_begin(&dll), &dll, &vals[3].e), CCC_RESULT_OK);
    CHECK(validate(&dll), true);
...
CHECK_BEGIN_STATIC_FN(dll_test_push_and_splice_range)
{
    ...
    enum check_result const t = create_list(&dll, UTIL_PUSH_BACK, 4, vals);
    CHECK(t, PASS);
    CHECK(splice_range(&dll, dll_elem_begin(&dll), &dll, &vals[1].e,
                       dll_sentinel_end(&dll)),
          CCC_RESULT_OK);
    CHECK(validate(&dll), true);
    CHECK(check_order(&dll, 4, (int[]){1, 2, 3, 0}), PASS);
    CHECK(splice_range(&dll, dll_elem_begin(&dll), &dll, &vals[2].e,
                       dll_sentinel_end(&dll)),
          CCC_RESULT_OK);
    CHECK(validate(&dll), true);
