...
CHECK_BEGIN_STATIC_FN(fpq_test_insert_remove_four_dups)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        ptrdiff_t const size_check = i + 1;
        CHECK(ccc_fpq_size(&fpq), size_check);
    }
    CHECK(ccc_fpq_size(&fpq), (ptrdiff_t)4);
    for (int i = 0; i < 4; ++i)
    {
        three_vals[i].val = 0;
        (void)pop(&fpq);
    ...
    CHECK(ccc_fpq_size(&fpq), (ptrdiff_t)0);
    CHECK_END_FN();
}
...
CHECK_BEGIN_STATIC_FN(fpq_test_insert_erase_shuffled)
{
    ...
    {
        ptrdiff_t const rand_index = rand_range(0, (int)ccc_fpq_size(&fpq) - 1);
