...
BEGIN_STATIC_TEST(check_range, flat_ordered_map const *const fom,
                  ...
                  int const expect_range[])
{
    ...
    END_FAIL({
        ...
        for (size_t j = 0; j < n && iter != end_range(r);
             ++j, iter = next(fom, &iter->elem))
        {
            if (iter == end(fom) || !iter)
            {
                return TEST_STATUS;
            }
        ...
        for (; iter != end_range(r); iter = next(fom, &iter->elem))
        {
            (void)fprintf(stderr, "%s%d, %s", RED, iter->id, NONE);
        }
...
BEGIN_STATIC_TEST(check_rrange, flat_ordered_map const *const fom,
                  rrange const *const r, size_t const n,
                  int const expect_rrange[])
{
    struct val *iter = rbegin_rrange(r);
    size_t index = 0;
    for (; iter != rend_rrange(r); iter = rnext(fom, &iter->elem))
    {
        int const cur_id = iter->id;
    ...
    END_FAIL({
        ...
        for (j = 0; j < n && iter != rend_rrange(r);
             ++j, iter = rnext(fom, &iter->elem))
        {
            if (iter == rend(fom) || !iter)
            {
                return TEST_STATUS;
            }
...
BEGIN_STATIC_TEST(iterator_check, flat_ordered_map *s)
{
...
BEGIN_STATIC_TEST(fomap_test_forward_iter)
{
    flat_ordered_map s
...
BEGIN_STATIC_TEST(fomap_test_iterate_removal)
{
    flat_ordered_map s
...
BEGIN_STATIC_TEST(fomap_test_iterate_remove_reinsert)
{
    flat_ordered_map s
        = fom_init((struct val[1001]){}, 1001, elem, id, NULL, val_cmp, NULL);
    /* Seed the test with any integer for reproducible random test sequence
...
BEGIN_STATIC_TEST(fomap_test_valid_range)
{
    flat_ordered_map s
...
BEGIN_STATIC_TEST(fomap_test_valid_range_equals)
{
    flat_ordered_map s
        = fom_init((struct val[26]){}, 26, elem, id, NULL, val_cmp, NULL);
    int const num_nodes = 25;
...
BEGIN_STATIC_TEST(fomap_test_invalid_range)
{
    flat_ordered_map s
        = fom_init((struct val[26]){}, 26, elem, id, NULL, val_cmp, NULL);
    int const num_nodes = 25;
...
BEGIN_STATIC_TEST(fomap_test_empty_range)
{
    flat_ordered_map s
        = fom_init((struct val[26]){}, 26, elem, id, NULL, val_cmp, NULL);
    int const num_nodes = 25;
...
main()
{
    return RUN_TESTS(fomap_test_forward_iter(), fomap_test_iterate_removal(),
                     fomap_test_valid_range(), fomap_test_valid_range_equals(),
                     fomap_test_invalid_range(), fomap_test_empty_range(),
                     fomap_test_iterate_remove_reinsert());
}
