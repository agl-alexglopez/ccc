...
CHECK_BEGIN_STATIC_FN(fomap_test_insert)
{
    ccc_flat_ordered_map fom
        = fom_init((struct val[10]){}, 10, elem, id, NULL, id_cmp, NULL);
...
CHECK_BEGIN_STATIC_FN(fomap_test_entry_api_functional)
{
    ...
    ccc_flat_ordered_map fom
        = fom_init((struct val[200]){}, 200, elem, id, NULL, id_cmp, NULL);
...
{
    ccc_flat_ordered_map fom
        = fom_init((struct val[20]){}, 20, elem, id, NULL, id_cmp, NULL);
...
CHECK_BEGIN_STATIC_FN(fomap_test_resize)
{
    ...
    ccc_flat_ordered_map fom
        = fom_init((struct val *)malloc(sizeof(struct val) * prime_start),
...
{
    ccc_flat_ordered_map fom
        = fom_init((struct val *)NULL, 0, elem, id, std_alloc, id_cmp, NULL);
    int const to_insert = 1000;
    int const larger_prime = 1009;
    for (int i = 0, shuffled_index = larger_prime % to_insert; i < to_insert;
         ++i, shuffled_index = (shuffled_index + larger_prime) % to_insert)
...
CHECK_BEGIN_STATIC_FN(fomap_test_insert_shuffle)
{
    ...
    ccc_flat_ordered_map s
        = fom_init((struct val[51]){}, 51, elem, id, NULL, id_cmp, NULL);
    CHECK(size > 1, true);
    int const prime = 53;
