...
fhash_test_insert_via_entry_macros(void)
{
    ...
    {
        struct val const *const d
            = FH_INS_ENT(FH_ENT(&fh, i), (struct val){i, i, {}});
        CHECK((d != NULL), true, "%d");
        CHECK(d->id, i, "%d");
        CHECK(d->val, i, "%d");
    }
    ...
    {
        struct val const *const d
            = FH_INS_ENT(FH_ENT(&fh, i), (struct val){i, i + 1, {}});
        /* All values in the array should be odd now */
        CHECK((d != NULL), true, "%d");
        CHECK(d->val, i + 1, "%d");
        if (i % 2)
...
fhash_test_entry_api_macros(void)
{
    ...
    for (int i = 0; i < size / 2; i += 2)
    {
        ...
        struct val const *const d
            = FH_OR_INS(FH_ENT(&fh, i), fhash_create(i, i));
        CHECK((d != NULL), true, "%d");
        CHECK(d->id, i, "%d");
        CHECK(d->val, i, "%d");
    }
    ...
    {
        struct val const *const d = FH_OR_INS(
            FH_AND_MODIFY(FH_ENT(&fh, i), fhash_modplus), fhash_create(i, i));
        /* All values in the array should be odd now */
        CHECK((d != NULL), true, "%d");
        CHECK(d->id, i, "%d");
        if (i % 2)
    ...
    for (int i = 0; i < size / 2; ++i)
    {
        FH_OR_INS(FH_ENT(&fh, i), (struct val){0})->val++;
        /* All values in the array should be odd now */
        CHECK(FH_OR_INS(FH_ENT(&fh, i), (struct val){0})->val % 2 == 0, true,
              "%d");
    }
    CHECK(ccc_fh_size(&fh), (size / 2), "%zu");
    return PASS;
...
fhash_test_two_sum(void)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        struct val const *const v = FH_GET(FH_ENT(&fh, target - addends[i]));
        if (v)
        {
            indices[0] = i;
            indices[1] = v->val;
        ...
        FH_INS_ENT(FH_ENT(&fh, addends[i]),
...
fhash_test_resize_macros(void)
{
    ...
    for (int i = 0, shuffled_index = larger_prime % to_insert; i < to_insert;
         ++i, shuffled_index = (shuffled_index + larger_prime) % to_insert)
    {
        struct val *v = FH_INS_ENT(FH_ENT(&fh, shuffled_index),
    ...
    {
        struct val const *const in_table
            = FH_OR_INS(FH_AND_MODIFY_W(FH_ENT(&fh, shuffled_index),
                                        fhash_swap_val, shuffled_index),
                        (struct val){0});
        CHECK(in_table != NULL, true, "%d");
        CHECK(in_table->val, shuffled_index, "%d");
        FH_OR_INS(FH_ENT(&fh, shuffled_index), (struct val){0})->val = i;
        struct val const *v = FH_GET(FH_ENT(&fh, shuffled_index));
        CHECK(v->val, i, "%d");
    }
    CHECK(ccc_fh_clear_and_free(&fh, NULL), CCC_OK, "%d");
    return PASS;
...
fhash_test_resize_from_null_macros(void)
{
         ...
         ++i, shuffled_index = (shuffled_index + larger_prime) % to_insert)
    {
        ...
        CHECK(in_table->val, shuffled_index, "%d");
        FH_OR_INS(FH_ENT(&fh, shuffled_index), (struct val){0})->val = i;
        struct val *v = FH_GET_MUT(FH_ENT(&fh, shuffled_index));
        CHECK(v->val, i, "%d");
    }
    CHECK(ccc_fh_clear_and_free(&fh, NULL), CCC_OK, "%d");
    return PASS;
...
fhash_test_insert_limit(void)
{
    ...
    in_table = FH_INS_ENT(FH_ENT(&fh, last_index),
    ...
    in_table = FH_INS_ENT(FH_ENT(&fh, shuffled_index),
...
fhash_test_insert_wrong_type(void)
{
    ...
    struct too_big const *wrong
        = FH_INS_ENT(FH_ENT(&fh, 137), (struct too_big){.a = {137, 137, {}}});
    CHECK(ccc_fh_size(&fh), 0, "%zu");
    CHECK(wrong == NULL, true, "%d");
    wrong = FH_OR_INS(FH_AND_MODIFY(FH_ENT(&fh, 137), fhash_modplus),
                      (struct too_big){.a = {.id = 137, .val = 137}});
    CHECK(wrong == NULL, true, "%d");
    CHECK(ccc_fh_size(&fh), 0, "%zu");
    struct val *correct
        = FH_INS_ENT(FH_ENT(&fh, 137), (struct val){137, 137, {}});
    ...
    CHECK(wrong == NULL, true, "%d");
    CHECK(ccc_fh_size(&fh), 1, "%zu");
    CHECK(((struct val *)FH_GET(FH_ENT(&fh, 137)))->val, 138, "%d");
    ...
    wrong = FH_INS_ENT(FH_AND_MODIFY(FH_ENT(&fh, 137), fhash_modplus),
                       (struct too_big){.a = {0, 0, {}}});
    CHECK(wrong == NULL, true, "%d");
    CHECK(ccc_fh_size(&fh), 1, "%zu");
    CHECK(((struct val *)FH_GET(FH_ENT(&fh, 137)))->val, 139, "%d");
    return PASS;
}
