...
check_static_begin(handle_ordered_map_test_empty)
{
...
check_static_begin(handle_ordered_map_test_copy_no_allocate)
{
...
check_static_begin(handle_ordered_map_test_copy_no_allocate_fail)
{
...
check_static_begin(handle_ordered_map_test_copy_allocate)
{
    Handle_ordered_map src = handle_ordered_map_initialize(
    ...
    check(is_empty(&dst), true);
    check_end({
        (void)handle_ordered_map_clear_and_free(&src, NULL);
        (void)handle_ordered_map_clear_and_free(&dst, NULL);
    });
}

check_static_begin(handle_ordered_map_test_copy_allocate_fail)
{
    Handle_ordered_map src = handle_ordered_map_initialize(
        NULL, struct Val, id, id_order, std_allocate, NULL, 0);
    Handle_ordered_map dst = handle_ordered_map_initialize(
    ...
    CCC_Result res = handle_ordered_map_copy(&dst, &src, NULL);
    check(res != CCC_RESULT_OK, true);
    check_end({ (void)handle_ordered_map_clear_and_free(&src, NULL); });
}

int
main()
{
    return check_run(handle_ordered_map_test_empty(),
                     handle_ordered_map_test_copy_no_allocate(),
                     handle_ordered_map_test_copy_no_allocate_fail(),
                     handle_ordered_map_test_copy_allocate(),
                     handle_ordered_map_test_copy_allocate_fail());
}
