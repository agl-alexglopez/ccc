...
hom_declare_fixed_map(lru_fixed_map, struct lru_elem, LRU_CAP);

/*===========================   Prototypes   ================================*/

static CCC_Order cmp_by_key(CCC_Key_comparator_context cmp);
static CCC_Order cmp_list_elems(CCC_Type_comparator_context ccmp);
...
static struct lru_cache lru_cache = {
    .map = hom_initialize(&(lru_fixed_map){}, struct lru_elem, key, cmp_by_key,
                          NULL, NULL, hom_fixed_capacity(lru_fixed_map)),
...
CHECK_BEGIN_STATIC_FN(run_lru_cache)
{
    ...
    CHECK_END_FN({ (void)CCC_hom_clear(&lru_cache.map, NULL); });
}

CHECK_BEGIN_STATIC_FN(lru_put, struct lru_cache *const lru, int const key,
                      int const val)
{
    CCC_homap_handle const *const ent = handle_r(&lru->map, &key);
    if (occupied(ent))
    {
        struct lru_elem *const found = hom_at(&lru->map, unwrap(ent));
        found->key = key;
        found->val = val;
                      ...
                      int *val)
{
    ...
    struct lru_elem *const found
        = hom_at(&lru->map, get_key_val(&lru->map, &key));
    if (!found)
    {
        *val = -1;
    }
