...
check_static_begin(handle_ordered_map_test_insert_erase_shuffled)
{
...
check_static_begin(handle_ordered_map_test_prime_shuffle)
{
    CCC_Handle_ordered_map s
        = handle_ordered_map_initialize(&(small_fixed_map){}, struct Val, id,
                                        id_order, NULL, NULL, SMALL_FIXED_CAP);
...
check_static_begin(handle_ordered_map_test_weak_srand)
{
...
check_static_begin(handle_ordered_map_test_insert_erase_cycles_no_allocate)
{
    CCC_Handle_ordered_map s = handle_ordered_map_initialize(
        &(standard_fixed_map){}, struct Val, id, id_order, NULL, NULL,
        STANDARD_FIXED_CAP);
...
check_static_begin(handle_ordered_map_test_insert_erase_cycles_allocate)
{
    CCC_Handle_ordered_map s = handle_ordered_map_initialize(
    ...
    check(is_empty(&s), true);
    check_end(handle_ordered_map_clear_and_free(&s, NULL););
}

int
main()
{
    return check_run(handle_ordered_map_test_insert_erase_shuffled(),
                     handle_ordered_map_test_prime_shuffle(),
                     handle_ordered_map_test_weak_srand(),
                     handle_ordered_map_test_insert_erase_cycles_no_allocate(),
                     handle_ordered_map_test_insert_erase_cycles_allocate());
}
