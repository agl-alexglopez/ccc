...
{
    flat_double_ended_queue q = fdeq_initialize((int[3]){}, int, NULL, NULL, 3);
...
{
    flat_double_ended_queue q = fdeq_initialize((int[2]){}, int, NULL, NULL, 2);
...
{
    flat_double_ended_queue q = fdeq_initialize((int[6]){}, int, NULL, NULL, 6);
    CHECK(create_queue(&q, 3, (int[3]){0, 1, 2}), PASS);
...
CHECK_BEGIN_STATIC_FN(fdeq_test_insert_ranges)
{
    flat_double_ended_queue q
        = fdeq_initialize(((int[6]){0, 1, 2}), int, NULL, NULL, 6, 3);
    CHECK(check_order(&q, 3, (int[3]){0, 1, 2}), PASS);
...
{
    flat_double_ended_queue q = fdeq_initialize(NULL, int, NULL, NULL, 0);
