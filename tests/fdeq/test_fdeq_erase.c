...
{
    flat_double_ended_queue q = fdeq_init((int[3]){}, 3, NULL, NULL);
...
{
    flat_double_ended_queue q = fdeq_init((int[6]){}, 6, NULL, NULL);
...
{
    flat_double_ended_queue q = fdeq_init((int[10]){}, 10, NULL, NULL);
    CHECK(create_queue(&q, 6, (int[6]){0, 1, 2, 3, 4, 5}), PASS);
