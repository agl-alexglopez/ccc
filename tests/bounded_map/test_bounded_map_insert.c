...
static inline void
bounded_map_modplus(CCC_Type_context const t)
{
    ((struct Val *)t.type)->val++;
}

check_static_begin(bounded_map_test_insert)
{
    CCC_Bounded_map rom = bounded_map_initialize(rom, struct Val, elem, key,
                                                 id_order, NULL, NULL);
...
check_static_begin(bounded_map_test_insert_macros)
{
    ...
    struct Val const *ins = CCC_bounded_map_or_insert_w(
        entry_r(&rom, &(int){2}), (struct Val){.key = 2, .val = 0});
    check(ins != NULL, true);
    check(validate(&rom), true);
    check(count(&rom).count, 1);
    ins = bounded_map_insert_entry_w(entry_r(&rom, &(int){2}),
                                     (struct Val){.key = 2, .val = 0});
    check(validate(&rom), true);
    check(ins != NULL, true);
    ins = bounded_map_insert_entry_w(entry_r(&rom, &(int){9}),
                                     (struct Val){.key = 9, .val = 1});
    check(validate(&rom), true);
    check(ins != NULL, true);
    ins = CCC_entry_unwrap(
        bounded_map_insert_or_assign_w(&rom, 3, (struct Val){.val = 99}));
    check(validate(&rom), true);
    ...
    ins = CCC_entry_unwrap(
        bounded_map_insert_or_assign_w(&rom, 3, (struct Val){.val = 98}));
    check(validate(&rom), true);
    check(ins == NULL, false);
    check(ins->val, 98);
    check(count(&rom).count, 3);
    ins = CCC_entry_unwrap(
        bounded_map_try_insert_w(&rom, 3, (struct Val){.val = 100}));
    check(ins == NULL, false);
    check(validate(&rom), true);
    check(ins->val, 98);
    check(count(&rom).count, 3);
    ins = CCC_entry_unwrap(
        bounded_map_try_insert_w(&rom, 4, (struct Val){.val = 100}));
    check(ins == NULL, false);
    check(validate(&rom), true);
    check(ins->val, 100);
    check(count(&rom).count, 4);
    check_end(CCC_bounded_map_clear(&rom, NULL););
}

check_static_begin(bounded_map_test_insert_overwrite)
{
    CCC_Bounded_map rom = bounded_map_initialize(rom, struct Val, elem, key,
                                                 id_order, NULL, NULL);
...
check_static_begin(bounded_map_test_insert_then_bad_ideas)
{
    CCC_Bounded_map rom = bounded_map_initialize(rom, struct Val, elem, key,
                                                 id_order, NULL, NULL);
    struct Val q = {.key = 137, .val = 99};
...
check_static_begin(bounded_map_test_entry_api_functional)
{
    ...
    for (size_t i = 0; i < size / 2; ++i)
    {
        ...
        struct Val const *const d = or_insert(
            bounded_map_and_modify_w(entry_r(&rom, &def.key), struct Val,
                                     {
                                         T->val++;
                                     }),
            &def.elem);
...
check_static_begin(bounded_map_test_insert_via_entry_macros)
{
    /* Over allocate size now because we don't want to worry about resizing. */
    size_t const size = 200;
    CCC_Bounded_map rom = bounded_map_initialize(rom, struct Val, elem, key,
...
check_static_begin(bounded_map_test_entry_api_macros)
{
    /* Over allocate size now because we don't want to worry about resizing. */
    int const size = 200;
    CCC_Bounded_map rom = bounded_map_initialize(rom, struct Val, elem, key,
    ...
    for (int i = 0; i < size / 2; i += 2)
    {
        ...
        struct Val const *const d = bounded_map_or_insert_w(
            entry_r(&rom, &i), bounded_map_create(i, i));
        check((d != NULL), true);
        check(d->key, i);
        check(d->val, i);
    }
    ...
    {
        struct Val const *const d = bounded_map_or_insert_w(
            and_modify(entry_r(&rom, &i), bounded_map_modplus),
            bounded_map_create(i, i));
        /* All values in the array should be odd now */
        check((d != NULL), true);
        check(d->key, i);
        if (i % 2)
    ...
    {
        struct Val *v
            = bounded_map_or_insert_w(entry_r(&rom, &i), (struct Val){});
        check(v != NULL, true);
        v->val++;
        /* All values in the array should be odd now */
...
check_static_begin(bounded_map_test_resize_macros)
{
    ...
    for (int i = 0, shuffled_index = larger_prime % to_insert; i < to_insert;
         ++i, shuffled_index = (shuffled_index + larger_prime) % to_insert)
    {
        struct Val const *const in_table = bounded_map_or_insert_w(
            bounded_map_and_modify_w(entry_r(&rom, &shuffled_index), struct Val,
                                     {
        ...
        check(in_table != NULL, true);
        check(in_table->val, shuffled_index);
        struct Val *v = bounded_map_or_insert_w(entry_r(&rom, &shuffled_index),
                                                (struct Val){});
        check(v == NULL, false);
        v->val = i;
...
{
    return check_run(
        bounded_map_test_insert(), bounded_map_test_insert_macros(),
        bounded_map_test_insert_and_find(), bounded_map_test_insert_overwrite(),
        bounded_map_test_insert_then_bad_ideas(),
        bounded_map_test_insert_via_entry(),
