...
CHECK_BEGIN_STATIC_FN(hromap_test_copy_no_alloc)
{
    handle_realtime_ordered_map src
        = hrm_init((struct val[11]){}, elem, id, id_cmp, nullptr, nullptr, 11);
    ...
    ccc_result res = hrm_copy(&dst, &src, nullptr);
...
CHECK_BEGIN_STATIC_FN(hromap_test_copy_no_alloc_fail)
{
    handle_realtime_ordered_map src
        = hrm_init((struct val[11]){}, elem, id, id_cmp, nullptr, nullptr, 11);
    handle_realtime_ordered_map dst
        = hrm_init((struct val[7]){}, elem, id, id_cmp, nullptr, nullptr, 7);
    (void)swap_handle(&src, &(struct val){.id = 0}.elem);
    (void)swap_handle(&src, &(struct val){.id = 1, .val = 1}.elem);
    (void)swap_handle(&src, &(struct val){.id = 2, .val = 2}.elem);
    CHECK(size(&src).count, 3);
...
{
    handle_realtime_ordered_map src = hrm_init((struct val *)nullptr, elem, id,
                                               id_cmp, std_alloc, nullptr, 0);
    handle_realtime_ordered_map dst = hrm_init((struct val *)nullptr, elem, id,
                                               id_cmp, std_alloc, nullptr, 0);
    (void)swap_handle(&src, &(struct val){.id = 0}.elem);
    (void)swap_handle(&src, &(struct val){.id = 1, .val = 1}.elem);
    (void)swap_handle(&src, &(struct val){.id = 2, .val = 2}.elem);
    ...
    CHECK(is_empty(&dst), true);
    CHECK_END_FN({
        (void)hrm_clear_and_free(&src, nullptr);
        (void)hrm_clear_and_free(&dst, nullptr);
    });
}
...
CHECK_BEGIN_STATIC_FN(hromap_test_copy_alloc_fail)
{
    ...
    ccc_result res = hrm_copy(&dst, &src, nullptr);
    CHECK(res != CCC_RESULT_OK, true);
    CHECK_END_FN({ (void)hrm_clear_and_free(&src, nullptr); });
}
