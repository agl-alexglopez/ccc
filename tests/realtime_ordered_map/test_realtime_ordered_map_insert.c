...
check_static_begin(realtime_ordered_map_test_insert)
{
...
check_static_begin(realtime_ordered_map_test_insert_macros)
{
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
    ...
    check_end(CCC_realtime_ordered_map_clear(&rom, NULL););
}

check_static_begin(realtime_ordered_map_test_insert_overwrite)
{
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
        rom, struct Val, elem, key, id_order, NULL, NULL);
...
check_static_begin(realtime_ordered_map_test_insert_then_bad_ideas)
{
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
        rom, struct Val, elem, key, id_order, NULL, NULL);
    struct Val q = {.key = 137, .val = 99};
...
check_static_begin(realtime_ordered_map_test_entry_api_functional)
{
    ...
    check(count(&rom).count, (size / 2));
    check_end(realtime_ordered_map_clear(&rom, NULL););
}

check_static_begin(realtime_ordered_map_test_insert_via_entry)
{
    /* Over allocate size now because we don't want to worry about resizing. */
    size_t const size = 200;
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
...
check_static_begin(realtime_ordered_map_test_insert_via_entry_macros)
{
    /* Over allocate size now because we don't want to worry about resizing. */
    size_t const size = 200;
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
...
check_static_begin(realtime_ordered_map_test_entry_api_macros)
{
    /* Over allocate size now because we don't want to worry about resizing. */
    int const size = 200;
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
...
check_static_begin(realtime_ordered_map_test_two_sum)
{
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
        rom, struct Val, elem, key, id_order, std_allocate, NULL);
...
check_static_begin(realtime_ordered_map_test_resize)
{
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
        rom, struct Val, elem, key, id_order, std_allocate, NULL);
...
check_static_begin(realtime_ordered_map_test_resize_macros)
{
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
        rom, struct Val, elem, key, id_order, std_allocate, NULL);
    int const to_insert = 1000;
...
check_static_begin(realtime_ordered_map_test_resize_frealtime_ordered_map_null)
{
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
        rom, struct Val, elem, key, id_order, std_allocate, NULL);
    int const to_insert = 1000;
...
check_static_begin(realtime_ordered_map_test_insert_and_find)
{
    int const size = 101;
    CCC_Realtime_ordered_map rom = realtime_ordered_map_initialize(
        rom, struct Val, elem, key, id_order, std_allocate, NULL);
...
check_static_begin(realtime_ordered_map_test_insert_shuffle)
{
...
check_static_begin(realtime_ordered_map_test_insert_weak_srand)
{
