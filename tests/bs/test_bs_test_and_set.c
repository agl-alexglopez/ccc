...
{
    bitset bs = bs_init((bitblock[bs_blocks(10)]){}, NULL, NULL, 10, 10);
    ptrdiff_t const larger_prime = 11;
    for (ptrdiff_t i = 0, shuf_i = larger_prime % 10; i < 10;
...
CHECK_BEGIN_STATIC_FN(bs_test_set_range)
{
    ...
    for (ptrdiff_t i = 0, end = 512; i < end; ++i, --end)
    {
        ptrdiff_t const count = end - i;
        CHECK(bs_set_range(&bs, i, count, CCC_TRUE), CCC_RESULT_OK);
        CHECK(bs_popcount_range(&bs, i, count), count);
        CHECK(bs_popcount(&bs), count);
        CHECK(bs_set_range(&bs, i, count, CCC_FALSE), CCC_RESULT_OK);
...
CHECK_BEGIN_STATIC_FN(bs_test_flip_range)
{
    bitset bs = bs_init((bitblock[bs_blocks(512)]){}, NULL, NULL, 512, 512);
    CHECK(bs_set_all(&bs, CCC_TRUE), CCC_RESULT_OK);
    ptrdiff_t const orignal_popcount = bs_popcount(&bs);
    /* Start with a full range and reduce from the end. */
    for (ptrdiff_t i = 0; i < 512; ++i)
    {
        ptrdiff_t const count = 512 - i;
...
CHECK_BEGIN_STATIC_FN(bs_test_any)
{
    bitset bs = bs_init((bitblock[bs_blocks(512)]){}, NULL, NULL, 512, 512);
    CHECK(bs_set_all(&bs, CCC_TRUE), CCC_RESULT_OK);
    ptrdiff_t const cap = bs_capacity(&bs);
    /* Start with a full range and reduce by moving start forward. */
    for (ptrdiff_t i = 0, end = 512; i < end; ++i, --end)
    {
        ptrdiff_t const count = end - i;
...
CHECK_BEGIN_STATIC_FN(bs_test_all)
{
    ...
    for (ptrdiff_t i = 1, end = 512; i < end; ++i, --end)
    {
        ptrdiff_t const count = end - i;
        CHECK(bs_set_range(&bs, i, count, CCC_TRUE), CCC_RESULT_OK);
        CHECK(bs_popcount_range(&bs, i, count), count);
...
CHECK_BEGIN_STATIC_FN(bs_test_first_trailing_one)
{
    ...
    CHECK(bs_set_all(&bs, CCC_TRUE), CCC_RESULT_OK);
    /* Start with an almost full range and reduce by moving start forward. */
    for (ptrdiff_t i = 0, end = 512; i < end - 1; ++i)
    {
...
{
    bitset bs = bs_init((bitblock[bs_blocks(512)]){}, NULL, NULL, 512, 512);
    ptrdiff_t window = sizeof(bitblock) * CHAR_BIT;
    /* Slide a group of int size as a window across the set. */
    for (ptrdiff_t i = 0; i < (512 - window - 1); ++i)
    {
...
{
    bitset bs = bs_init((bitblock[bs_blocks(512)]){}, NULL, NULL, 512, 512);
    ptrdiff_t const end = bs_blocks(512);
    ptrdiff_t const bits_in_block = sizeof(bitblock) * CHAR_BIT;
    ptrdiff_t const first_half = bits_in_block / 2;
    ptrdiff_t const second_half = first_half - 1;
    ...
    for (ptrdiff_t block = 0, i = 0; block < end;
...
CHECK_BEGIN_STATIC_FN(bs_test_first_leading_ones)
{
    ...
    ptrdiff_t window = sizeof(bitblock) * CHAR_BIT;
    /* Slide a group of int size as a window across the set. */
    for (ptrdiff_t i = 511; i > window + 1; --i)
    {
                    ...
                    ptrdiff_t const col_start)
{
    ...
    for (ptrdiff_t r = row_start; r < row_start + 3; ++r)
    {
        for (ptrdiff_t c = col_start; c < col_start + 3; ++c)
        {
            if (!board[r][c])
            {
                continue;
...
CHECK_BEGIN_STATIC_FN(bs_test_valid_sudoku)
{
    ...
    ,{0,0,0, 0,8,0, 0,7,9}};
    /* clang-format on */
    bitset row_check = bs_init((bitblock[bs_blocks(9L * 9L)]){}, NULL, NULL,
                               9L * 9L, 9L * 9L);
    bitset col_check = bs_init((bitblock[bs_blocks(9L * 9L)]){}, NULL, NULL,
                               9L * 9L, 9L * 9L);
    ptrdiff_t const box_step = 3;
    for (ptrdiff_t row = 0; row < 9; row += box_step)
    {
        for (ptrdiff_t col = 0; col < 9; col += box_step)
        {
