...
CHECK_BEGIN_STATIC_FN(sll_test_push_and_splice)
{
    ...
    CHECK(splice(&sll, sll_elem_begin(&sll), &sll, &vals[0].e), CCC_RESULT_OK);
    CHECK(validate(&sll), true);
...
CHECK_BEGIN_STATIC_FN(sll_test_push_and_splice_range)
{
    ...
    CHECK(splice_range(&sll, sll_sentinel_begin(&sll), &sll, &vals[2].e,
                       &vals[0].e),
          CCC_RESULT_OK);
    ...
    CHECK(splice_range(&sll, &vals[3].e, &sll, &vals[2].e, &vals[0].e),
          CCC_RESULT_OK);
    CHECK(check_order(&sll, 5, (int[5]){4, 3, 2, 1, 0}), PASS);
    CHECK(splice_range(&sll, sll_sentinel_begin(&sll), &sll, &vals[3].e,
                       &vals[1].e),
          CCC_RESULT_OK);
...
CHECK_BEGIN_STATIC_FN(sll_test_sort_insert)
{
    ...
    CHECK(r, CCC_RESULT_OK);
    CHECK(validate(&sll), true);
    CHECK(check_order(&sll, 9, (int[9]){-99, -55, 1, 1, 2, 4, 5, 9, 99}), PASS);
...
main()
{
    return CHECK_RUN(sll_test_insert_three(), sll_test_push_and_splice(),
                     ...
                     sll_test_sort_halves(), sll_test_sort_insert());
}
