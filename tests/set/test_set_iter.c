...
set_test_iterate_remove_reinsert(void)
{
         ...
         i != ccc_set_end(&s); i = next)
    {
        ...
        {
            (void)ccc_set_erase(&s, i);
            struct val *v = CCC_SET_OF(struct val, elem, i);
...
set_test_valid_range(void)
{
    ...
    CHECK(CCC_SET_OF(struct val, elem, ccc_set_begin_range(&range))->val,
          range_vals[0], int, "%d");
    CHECK(CCC_SET_OF(struct val, elem, ccc_set_end_range(&range))->val,
          range_vals[7], int, "%d");
    size_t index = 0;
    ccc_set_elem *i1 = ccc_set_begin_range(&range);
    for (; i1 != ccc_set_end_range(&range); i1 = ccc_set_next(&s, i1))
    {
        int const cur_val = CCC_SET_OF(struct val, elem, i1)->val;
        CHECK(range_vals[index], cur_val, int, "%d");
        ++index;
    }
    CHECK(i1, ccc_set_end_range(&range), ccc_set_elem *, "%p");
    CHECK(CCC_SET_OF(struct val, elem, i1)->val, range_vals[7], int, "%d");
    b.val = 119;
    e.val = 84;
    ...
    CHECK(CCC_SET_OF(struct val, elem, ccc_set_begin_rrange(&rev_range))->val,
          rev_range_vals[0], int, "%d");
    CHECK(CCC_SET_OF(struct val, elem, ccc_set_end_rrange(&rev_range))->val,
    ...
    {
        int const cur_val = CCC_SET_OF(struct val, elem, i2)->val;
        CHECK(rev_range_vals[index], cur_val, int, "%d");
        ++index;
    }
    CHECK(i2, ccc_set_end_rrange(&rev_range), ccc_set_elem *, "%p");
    CHECK(CCC_SET_OF(struct val, elem, i2)->val, rev_range_vals[7], int, "%d");
    return PASS;
}
...
set_test_empty_range(void)
{
    ...
    CHECK(
        CCC_SET_OF(struct val, elem, ccc_set_begin_range(&forward_range))->val,
        vals[0].val, int, "%d");
    CHECK(CCC_SET_OF(struct val, elem, ccc_set_end_range(&forward_range))->val,
          vals[0].val, int, "%d");
    b.val = 150;
    e.val = 999;
    ccc_set_rrange const rev_range = ccc_set_equal_rrange(&s, &b.elem, &e.elem);
