...
set_test_forward_iter(void)
{
    ...
    int j = 0;
    for (struct val *e = ccc_set_begin(&s); e; e = ccc_set_next(&s, e), ++j)
    {}
    CHECK(j, 0, int, "%d");
...
set_test_valid_range(void)
{
    ...
    ccc_range const range = ccc_set_equal_range(&s, &b, &e);
    ...
    ccc_rrange const rev_range = ccc_set_equal_rrange(&s, &b, &e);
...
set_test_invalid_range(void)
{
    ...
    ccc_range const rev_range = ccc_set_equal_range(&s, &b, &e);
    ...
    ccc_rrange const range = ccc_set_equal_rrange(&s, &b, &e);
...
set_test_empty_range(void)
{
    ...
    ccc_range const forward_range = ccc_set_equal_range(&s, &b, &e);
...
iterator_check(ccc_set *s)
{
    ...
    iter_count = 0;
    for (struct val *e = ccc_set_rbegin(s); e; e = ccc_set_rnext(s, e))
    {
        ++iter_count;
