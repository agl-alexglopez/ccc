...
test_push(void)
{
    ...
    for (size_t n = step; n < end_size; n += step)
    {
        ...
        ccc_ordered_multimap omm = ccc_omm_init(
            omm, struct val, ommap_elem, val, val_key_cmp, nullptr, nullptr);
        ccc_priority_queue pq = ccc_pq_init(struct val, pq_elem, CCC_LES,
                                            val_cmp, nullptr, nullptr);
        clock_t begin = clock();
        ...
        ccc_flat_priority_queue fpq = ccc_fpq_init(val_array, CCC_LES, val_cmp,
                                                   nullptr, nullptr, n + 1);
        begin = clock();
        for (size_t i = 0; i < n; ++i)
        {
