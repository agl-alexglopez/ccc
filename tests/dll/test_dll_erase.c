...
BEGIN_STATIC_TEST(dll_test_push_pop_middle_range)
{
    ...
    CHECK(t, PASS);
    (void)dll_erase_range(&dll, &vals[1].e, &vals[4].e);
    CHECK(validate(&dll), true);
    CHECK(size(&dll), 2);
    CHECK(check_order(&dll, 2, (int[2]){0, 4}), PASS);
    (void)dll_erase_range(&dll, &vals[0].e, dll_end_sentinel(&dll));
    CHECK(validate(&dll), true);
    CHECK(size(&dll), 0);
    END_TEST();
}
