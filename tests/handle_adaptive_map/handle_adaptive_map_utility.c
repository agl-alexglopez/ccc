...
check_begin(insert_shuffled, CCC_Handle_adaptive_map *m, size_t const size,
...
size_t
inorder_fill(int vals[], size_t size, CCC_Handle_adaptive_map const *const m)
{
    ...
    {
        struct Val const *const v = CCC_handle_adaptive_map_at(m, e);
        vals[i++] = v->id;
    }
    return i;
}
