    ...
    Range const *const r, size_t const n, int const expect_range[])
{
    ...
    check_fail_end({
             ...
             ++j, iter = next(handle_realtime_ordered_map, iter))
        {
            ...
            {
                (void)fprintf(stderr, "%s%d, %s", CHECK_RED, iter->id,
                              CHECK_NONE);
            }
        }
    ...
    int const expect_range_reverse[])
{
    ...
    for (; iter != range_reverse_end(r);
         iter = reverse_next(handle_realtime_ordered_map, iter))
    {
        int const cur_id = iter->id;
    ...
    if (iter != reverse_end(handle_realtime_ordered_map))
    {
    ...
    check_fail_end({
        ...
        for (j = 0; j < n && iter != range_reverse_end(r);
             ++j, iter = reverse_next(handle_realtime_ordered_map, iter))
        {
            if (iter == reverse_end(handle_realtime_ordered_map) || !iter)
            {
                return CHECK_STATUS;
            }
...
check_static_begin(iterator_check, Handle_realtime_ordered_map *s)
{
...
check_static_begin(handle_realtime_ordered_map_test_forward_iter)
{
...
check_static_begin(handle_realtime_ordered_map_test_iterate_removal)
{
    CCC_Handle_realtime_ordered_map s = handle_realtime_ordered_map_initialize(
...
check_static_begin(handle_realtime_ordered_map_test_iterate_remove_reinsert)
{
    CCC_Handle_realtime_ordered_map s = handle_realtime_ordered_map_initialize(
        &(standard_fixed_map){}, struct Val, id, id_order, NULL, NULL,
        STANDARD_FIXED_CAP);
...
check_static_begin(handle_realtime_ordered_map_test_valid_range)
{
    CCC_Handle_realtime_ordered_map s = handle_realtime_ordered_map_initialize(
        &(small_fixed_map){}, struct Val, id, id_order, NULL, NULL,
        SMALL_FIXED_CAP);
...
check_static_begin(handle_realtime_ordered_map_test_valid_range_equals)
{
    CCC_Handle_realtime_ordered_map s = handle_realtime_ordered_map_initialize(
        &(small_fixed_map){}, struct Val, id, id_order, NULL, NULL,
        SMALL_FIXED_CAP);
...
check_static_begin(handle_realtime_ordered_map_test_invalid_range)
{
    CCC_Handle_realtime_ordered_map s = handle_realtime_ordered_map_initialize(
        &(small_fixed_map){}, struct Val, id, id_order, NULL, NULL,
        SMALL_FIXED_CAP);
...
check_static_begin(handle_realtime_ordered_map_test_empty_range)
{
    CCC_Handle_realtime_ordered_map s = handle_realtime_ordered_map_initialize(
        &(small_fixed_map){}, struct Val, id, id_order, NULL, NULL,
        SMALL_FIXED_CAP);
