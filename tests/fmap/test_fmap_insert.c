...
BEGIN_STATIC_TEST(fmap_test_insert_one)
{
    flat_ordered_map s = fom_init((struct val[2]){}, 2, struct val, elem, id,
                                  NULL, val_cmp, NULL);
    CHECK(occupied(insert_vr(&s, &(struct val){}.elem, &(struct val){}.elem)),
          false);
    CHECK(fom_empty(&s), false);
...
// {
//     /* This is also a good test to see if the buffer can manage its own
//     memory
//        when provided with a realloc function starting from NULL. */
//     flat_ordered_map s
//         = fom_init(NULL, 0, struct val, elem, id, realloc, val_cmp, NULL);
...
BEGIN_STATIC_TEST(fmap_test_insert_shuffle)
{
    size_t const size = 50;
    ccc_flat_ordered_map s = fom_init((struct val[51]){}, 51, struct val, elem,
...
BEGIN_STATIC_TEST(fmap_test_insert_weak_srand)
{
    int const num_nodes = 1000;
    ccc_flat_ordered_map s = fom_init((struct val[1001]){}, 1001, struct val,
...
main()
{
    return RUN_TESTS(fmap_test_insert_one(), /* fmap_test_insert_macros() ,*/
                     fmap_test_insert_shuffle(), fmap_test_insert_weak_srand());
}
