...
array_bounded_map_create(int const id, int const val)
{
    return (struct Val){.id = id, .val = val};
}

static inline void
array_bounded_map_modplus(CCC_Type_context const t)
{
...
check_static_begin(array_bounded_map_test_insert_macros)
{
    CCC_Array_bounded_map map
        = array_bounded_map_initialize(&(Small_fixed_map){}, struct Val, id,
                                       id_order, NULL, NULL, SMALL_FIXED_CAP);
    ...
    check(validate(&map), true);
    check(ins != NULL, true);
    ins = array_bounded_map_at(&map,
                               unwrap(array_bounded_map_insert_or_assign_with(
    ...
    ins = array_bounded_map_at(
        &map, CCC_array_unwrap(array_bounded_map_insert_or_assign_with(
                  &map, 3, (struct Val){.val = 98})));
    check(validate(&map), true);
    check(ins == NULL, false);
    check(ins->val, 98);
    check(count(&map).count, 3);
    ins = array_bounded_map_at(&map, unwrap(array_bounded_map_try_insert_with(
    ...
    check(count(&map).count, 3);
    ins = array_bounded_map_at(
        &map, CCC_array_unwrap(array_bounded_map_try_insert_with(
...
check_static_begin(array_bounded_map_test_insert_overwrite)
{
    CCC_Array_bounded_map map
        = array_bounded_map_initialize(&(Small_fixed_map){}, struct Val, id,
                                       id_order, NULL, NULL, SMALL_FIXED_CAP);
    ...
    v = array_bounded_map_at(&map, unwrap(&in_table));
    check(v != NULL, true);
    check(v->val, 100);
    check(q.val, 99);
...
check_static_begin(array_bounded_map_test_insert_then_bad_ideas)
{
    CCC_Array_bounded_map map
        = array_bounded_map_initialize(&(Small_fixed_map){}, struct Val, id,
                                       id_order, NULL, NULL, SMALL_FIXED_CAP);
    ...
    v = array_bounded_map_at(&map, unwrap(&hndl));
    check(v != NULL, true);
    check(v->val, 100);
    check(q.val, 99);
    q.val -= 9;

    v = array_bounded_map_at(&map, get_key_value(&map, &q.id));
    check(v != NULL, true);
    check(v->val, 100);
    check(q.val, 90);
    check_end();
...
check_static_begin(array_bounded_map_test_array_api_functional)
{
    /* Over allocate size now because we don't want to worry about resizing. */
    CCC_Array_bounded_map map = array_bounded_map_initialize(
...
check_static_begin(array_bounded_map_test_insert_via_handle)
{
    /* Over allocate size now because we don't want to worry about resizing. */
    CCC_Array_bounded_map map = array_bounded_map_initialize(
        &(Standard_fixed_map){}, struct Val, id, id_order, NULL, NULL,
...
check_static_begin(array_bounded_map_test_insert_via_array_macros)
{
    /* Over allocate size now because we don't want to worry about resizing. */
    CCC_Array_bounded_map map = array_bounded_map_initialize(
        &(Standard_fixed_map){}, struct Val, id, id_order, NULL, NULL,
...
check_static_begin(array_bounded_map_test_array_api_macros)
{
    /* Over allocate size now because we don't want to worry about resizing. */
    CCC_Array_bounded_map map = array_bounded_map_initialize(
        &(Standard_fixed_map){}, struct Val, id, id_order, NULL, NULL,
...
check_static_begin(array_bounded_map_test_two_sum)
{
    CCC_Array_bounded_map map
        = array_bounded_map_initialize(&(Small_fixed_map){}, struct Val, id,
                                       id_order, NULL, NULL, SMALL_FIXED_CAP);
    ...
    {
        struct Val const *const other_addend = array_bounded_map_at(
...
check_static_begin(array_bounded_map_test_resize)
{
    CCC_Array_bounded_map map = array_bounded_map_initialize(
...
check_static_begin(array_bounded_map_test_reserve)
{
    int const to_insert = 1000;
    struct Stack_allocator allocator
        = stack_allocator_initialize(Standard_fixed_map, 1);
    CCC_Array_bounded_map map = array_bounded_map_with_capacity(
        struct Val, id, id_order, stack_allocator_allocate, &allocator,
        STANDARD_FIXED_CAP - 1);
    check(array_bounded_map_capacity(&map).count >= STANDARD_FIXED_CAP - 1,
          true);
    int const larger_prime = 1009;
    for (int i = 0, shuffled_index = larger_prime % to_insert; i < to_insert;
         ++i, shuffled_index = (shuffled_index + larger_prime) % to_insert)
...
check_static_begin(array_bounded_map_test_resize_macros)
{
    CCC_Array_bounded_map map = array_bounded_map_initialize(
        NULL, struct Val, id, id_order, std_allocate, NULL, 0);
    int const to_insert = 1000;
...
check_static_begin(array_bounded_map_test_resize_from_null)
{
    CCC_Array_bounded_map map = array_bounded_map_initialize(
        NULL, struct Val, id, id_order, std_allocate, NULL, 0);
    int const to_insert = 1000;
...
check_static_begin(array_bounded_map_test_resize_from_null_macros)
{
    CCC_Array_bounded_map map = array_bounded_map_initialize(
        NULL, struct Val, id, id_order, std_allocate, NULL, 0);
    int const to_insert = 1000;
...
check_static_begin(array_bounded_map_test_insert_limit)
{
...
check_static_begin(array_bounded_map_test_insert_and_find)
{
    int const size = SMALL_FIXED_CAP;
    CCC_Array_bounded_map map
        = array_bounded_map_initialize(&(Small_fixed_map){}, struct Val, id,
    ...
    for (int i = 0; i < size; i += 2)
    {
        ...
        check(validate(&map), true);
        struct Val const *const v = array_bounded_map_at(&map, unwrap(&e));
        check(v == NULL, false);
        check(v->id, i);
        check(v->val, i);
    }
...
check_static_begin(array_bounded_map_test_insert_shuffle)
{
...
check_static_begin(array_bounded_map_test_insert_weak_srand)
{
...
{
    return check_run(
        array_bounded_map_test_insert(), array_bounded_map_test_insert_macros(),
        array_bounded_map_test_insert_and_find(),
        array_bounded_map_test_insert_overwrite(),
        array_bounded_map_test_insert_then_bad_ideas(),
        array_bounded_map_test_insert_via_handle(),
        array_bounded_map_test_insert_via_array_macros(),
        array_bounded_map_test_reserve(),
        array_bounded_map_test_array_api_functional(),
        array_bounded_map_test_array_api_macros(),
        array_bounded_map_test_two_sum(), array_bounded_map_test_resize(),
        array_bounded_map_test_resize_macros(),
        array_bounded_map_test_resize_from_null(),
        array_bounded_map_test_resize_from_null_macros(),
        array_bounded_map_test_insert_limit(),
        array_bounded_map_test_insert_weak_srand(),
        array_bounded_map_test_insert_shuffle());
}
