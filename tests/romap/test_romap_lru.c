...
static struct lru_cache lru_cache = {
    .cap = 3,
    .l = dll_init(lru_cache.l, struct lru_elem, list_elem, cmp_list_elems,
                  nullptr, nullptr),
    .map = rom_init(lru_cache.map, struct lru_elem, map_elem, key, cmp_by_key,
                    std_alloc, nullptr),
};
...
CHECK_BEGIN_STATIC_FN(run_lru_cache)
{
    ...
    CHECK_END_FN({ (void)ccc_rom_clear(&lru_cache.map, nullptr); });
}
