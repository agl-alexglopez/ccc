...
check_static_begin(fill_n, CCC_Handle_bounded_map *const handle_bounded_map,
...
check_static_begin(handle_bounded_map_test_validate)
{
    ...
    hndl = swap_handle(&handle_bounded_map, &(struct Val){.id = -1, .val = -1});
    check(validate(&handle_bounded_map), true);
...
check_static_begin(handle_bounded_map_test_insert)
{
    ...
    i += (size / 2);
    hndl = swap_handle(&handle_bounded_map, &(struct Val){.id = i, .val = i});
    check(validate(&handle_bounded_map), true);
    check(occupied(&hndl), false);
...
check_static_begin(handle_bounded_map_test_remove)
{
    ...
    int size = 30;
    CCC_Handle hndl
        = CCC_remove(&handle_bounded_map, &(struct Val){.id = -1, .val = -1});
    check(validate(&handle_bounded_map), true);
    check(occupied(&hndl), false);
    ...
    check(count(&handle_bounded_map).count, 1);
    struct Val old = {.id = -1};
    hndl = CCC_remove(&handle_bounded_map, &old);
    check(validate(&handle_bounded_map), true);
    check(occupied(&hndl), true);
    check(count(&handle_bounded_map).count, 0);
    check(old.val, -1);
    ...
    i += (size / 2);
    hndl = CCC_remove(&handle_bounded_map, &(struct Val){.id = i, .val = i});
    check(validate(&handle_bounded_map), true);
    check(occupied(&hndl), false);
    check(count(&handle_bounded_map).count, i);
    hndl = swap_handle(&handle_bounded_map, &(struct Val){.id = i, .val = i});
    check(validate(&handle_bounded_map), true);
    check(occupied(&hndl), false);
...
check_static_begin(handle_bounded_map_test_try_insert)
{
    ...
    int size = 30;
    CCC_Handle hndl
        = try_insert(&handle_bounded_map, &(struct Val){.id = -1, .val = -1});
    check(validate(&handle_bounded_map), true);
    check(occupied(&hndl), false);
    check(count(&handle_bounded_map).count, 1);
    hndl = try_insert(&handle_bounded_map, &(struct Val){.id = -1, .val = -1});
    check(validate(&handle_bounded_map), true);
    check(occupied(&hndl), true);
    check(count(&handle_bounded_map).count, 1);
    struct Val *v = handle_bounded_map_at(&handle_bounded_map, unwrap(&hndl));
    ...
    i += (size / 2);
    hndl = try_insert(&handle_bounded_map, &(struct Val){.id = i, .val = i});
    check(validate(&handle_bounded_map), true);
    check(occupied(&hndl), false);
    check(count(&handle_bounded_map).count, i + 2);
    hndl = try_insert(&handle_bounded_map, &(struct Val){.id = i, .val = i});
...
check_static_begin(handle_bounded_map_test_insert_or_assign)
{
    ...
    check(occupied(&hndl), false);
    check(count(&handle_bounded_map).count, 1);
    hndl = insert_or_assign(&handle_bounded_map,
...
main(void)
{
    return check_run(handle_bounded_map_test_insert(),
                     handle_bounded_map_test_remove(),
                     handle_bounded_map_test_validate(),
                     handle_bounded_map_test_try_insert(),
                     handle_bounded_map_test_try_insert_with(),
                     handle_bounded_map_test_insert_or_assign(),
                     handle_bounded_map_test_insert_or_assign_with(),
                     handle_bounded_map_test_handle_and_modify(),
                     handle_bounded_map_test_handle_and_modify_context(),
                     handle_bounded_map_test_handle_and_modify_with(),
                     handle_bounded_map_test_or_insert(),
                     handle_bounded_map_test_or_insert_with(),
                     handle_bounded_map_test_insert_handle(),
                     handle_bounded_map_test_insert_handle_with(),
                     handle_bounded_map_test_remove_handle());
}
