                   ...
                   int const expect_range[])
{
    ...
    check_fail_end({
        ...
        for (size_t j = 0; j < n && iterator != range_end(r);
             ++j, iterator = next(handle_bounded_map, iterator))
        {
            if (iterator == end(handle_bounded_map) || !iterator)
            {
                return CHECK_STATUS;
            }
        ...
        for (; iterator != range_end(r);
             iterator = next(handle_bounded_map, iterator))
        {
            (void)fprintf(stderr, "%s%d, %s", CHECK_RED, iterator->id,
                          CHECK_NONE);
        }
                   ...
                   int const expect_range_reverse[])
{
    ...
    for (; iterator != range_reverse_end(r);
         iterator = reverse_next(handle_bounded_map, iterator))
    {
        int const cur_id = iterator->id;
    ...
    if (iterator != reverse_end(handle_bounded_map))
    {
    ...
    check_fail_end({
        ...
        for (j = 0; j < n && iterator != range_reverse_end(r);
             ++j, iterator = reverse_next(handle_bounded_map, iterator))
        {
            if (iterator == reverse_end(handle_bounded_map) || !iterator)
            {
                return CHECK_STATUS;
            }
