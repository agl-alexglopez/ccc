...
check_static_begin(handle_bounded_map_test_insert_macros)
{
    ...
    check(validate(&handle_bounded_map), true);
    check(ins->val, 100);
    check(count(&handle_bounded_map).count, 4);
    check_end(clear_and_free(&handle_bounded_map, NULL););
}
...
check_static_begin(handle_bounded_map_test_insert_overwrite)
{
    ...
    CCC_Handle in_table = swap_handle(&handle_bounded_map, &q);
    check(occupied(&in_table), true);

    /* The old contents are now in q and the handle is in the table. */
    v = handle_bounded_map_at(&handle_bounded_map, unwrap(&in_table));
    check(v != NULL, true);
    check(v->val, 100);
    check(q.val, 99);
...
check_static_begin(handle_bounded_map_test_insert_then_bad_ideas)
{
    ...
    hndl = swap_handle(&handle_bounded_map, &q);
...
check_static_begin(handle_bounded_map_test_handle_api_functional)
{
    ...
    CCC_Handle_bounded_map handle_bounded_map = handle_bounded_map_initialize(
    ...
    check(count(&handle_bounded_map).count, (size / 2) / 2);
    ...
    check(count(&handle_bounded_map).count, (size / 2));
...
check_static_begin(handle_bounded_map_test_two_sum)
{
    ...
    for (size_t i = 0; i < (size_t)(sizeof(addends) / sizeof(addends[0])); ++i)
    {
        ...
        CCC_Handle const e = insert_or_assign(
            &handle_bounded_map, &(struct Val){.id = addends[i], .val = i});
...
check_static_begin(handle_bounded_map_test_resize)
{
    ...
    check(count(&handle_bounded_map).count, to_insert);
    for (int i = 0, shuffled_index = larger_prime % to_insert; i < to_insert;
         ++i, shuffled_index = (shuffled_index + larger_prime) % to_insert)
    {
    ...
    check(clear_and_free(&handle_bounded_map, NULL), CCC_RESULT_OK);
    check_end();
}
...
check_static_begin(handle_bounded_map_test_reserve)
{
    ...
    CCC_Result const r = handle_bounded_map_reserve(&handle_bounded_map,
    ...
    check_end(clear_and_free_reserve(&handle_bounded_map, NULL, std_allocate););
}
...
check_static_begin(handle_bounded_map_test_insert_limit)
{
    ...
    size_t const final_size = count(&handle_bounded_map).count;
    /* The last successful handle is still in the table and is overwritten. */
    struct Val v = {.id = last_index, .val = -1};
    CCC_Handle hndl = swap_handle(&handle_bounded_map, &v);
    ...
    hndl = swap_handle(&handle_bounded_map, &v);
    check(unwrap(&hndl) == 0, true);
...
check_static_begin(handle_bounded_map_test_insert_and_find)
{
    ...
    {
        CCC_Handle e
            = try_insert(&handle_bounded_map, &(struct Val){.id = i, .val = i});
        check(occupied(&e), false);
        check(validate(&handle_bounded_map), true);
        e = try_insert(&handle_bounded_map, &(struct Val){.id = i, .val = i});
        check(occupied(&e), true);
        check(validate(&handle_bounded_map), true);
        struct Val const *const v
            = handle_bounded_map_at(&handle_bounded_map, unwrap(&e));
        check(v == NULL, false);
        check(v->id, i);
        check(v->val, i);
    }
...
check_static_begin(handle_bounded_map_test_insert_shuffle)
{
    ...
    check(size > 1, true);
    int const prime = 67;
    check(insert_shuffled(&handle_bounded_map, size, prime), CHECK_PASS);
    int sorted_check[SMALL_FIXED_CAP - 1];
    check(inorder_fill(sorted_check, size, &handle_bounded_map), size);
...
check_static_begin(handle_bounded_map_test_insert_weak_srand)
{
    ...
    {
        CCC_Handle const e
            = swap_handle(&handle_bounded_map,
                          &(struct Val){.id = rand() /* NOLINT */, .val = i});
        check(insert_error(&e), false);
        check(validate(&handle_bounded_map), true);
    }
    check(count(&handle_bounded_map).count, (size_t)num_nodes);
    check_end();
}
